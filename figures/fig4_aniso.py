...
if not exists(dfile):
    ...
    vel = np.stack((vx_grid, vy_grid, vz_grid), axis=-1).reshape(N_bins**2, 3)
    pos_tiled = np.tile(pos[None], reps=[N_bins**2, 1])

    # exact DF
    f_exact = calc_DF_aniso(pos_tiled, vel, M, a).reshape(N_bins, N_bins) * vT_cen
    ...
    f_model = calc_DF_ensemble(pos_tiled, vel, u_q=u_q, u_p=u_p, flows=flows)
    ...
    # get f_data
    f_data = np.histogram2d(vR, vT, bins=[vR_edges, vT_edges], density=True)[0]
    ...
    np.savez(dfile, f_exact=f_exact, f_model=f_model, f_data=f_data, res=res)
